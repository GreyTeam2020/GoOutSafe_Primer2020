...
def index():
    if current_user is not None and hasattr(current_user, "id"):
        ...
        if len(request.form.get('time').split(":")) == 1:
            ...
            py_datetime = datetime.datetime(int (request.form.get('date').split("/")[2].strip()), int (request.form.get('date').split("/")[1].strip()), int (request.form.get('date').split("/")[0].strip()), int (request.form.get('time').split(":")[0].strip()), int (request.form.get('time').split(":")[1].strip()))
        
        #if user wants to book in the past..
        if py_datetime < datetime.datetime.now():
            return render_template("booking.html", success=False, error="You can not book in the past!")
        ...
        opening_hour = db.session.query(OpeningHours).filter_by(restaurant_id = request.form.get('restaurantID')).filter_by(week_day = week_day).filter(or_(OpeningHours.open_lunch == datetime.time(hour=12), OpeningHours.open_lunch == datetime.time(hour=20))).first()
        
        #the restaurant is closed
        if opening_hour is None:
            print("No Opening hour")
            return render_template("booking.html", success=False, error="The restaurant is closed")
        ...
        if (opening_hour.open_lunch is None or opening_hour.close_lunch is None) and (only_time<opening_hour.open_dinner or only_time>opening_hour.close_dinner):
            return render_template("booking.html", success=False, error="The restaurant is closed")

        if (opening_hour.open_dinner is None or opening_hour.close_dinner is None) and (only_time<opening_hour.open_lunch or only_time>opening_hour.close_lunch):
            return render_template("booking.html", success=False, error="The restaurant is closed") 
        #
        ...
        if opening_hour.open_lunch is not None and opening_hour.open_dinner is not None:
            ...
            if opening_hour.open_dinner > only_time and opening_hour.close_lunch < only_time:
                print("between")
                return render_template("booking.html", success=False, error="The restaurant is closed")
        ...
        test_hour = datetime.datetime.combine(py_datetime.date(), opening_hour.close_lunch)
        print (test_hour)
        ...
        if only_time < opening_hour.close_lunch:
            ...
            reservations = db.session.query(RestaurantTable.id).join(Reservation, RestaurantTable.id == Reservation.table_id).filter(RestaurantTable.restaurant_id == request.form.get('restaurantID')).filter(Reservation.reservation_date <= test_hour).filter(RestaurantTable.max_seats >= int (request.form.get('people')))
        else:
